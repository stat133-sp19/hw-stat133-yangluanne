...
PT2_2 <- PT2_1 %>%
  mutate(perc_of_total = total / PT_1$total * 100, perc_made_of_total = made / PT_1$made * 100) %>%
  select(name, perc_of_total, perc_made_of_total)
PT3_3 <- PT3_1 %>%
  mutate(perc_of_total = total / PT_1$total * 100, perc_made_of_total = made / PT_1$made * 100) %>%
  select(name, perc_of_total, perc_made_of_total)
```
...
# close to hoop shots
andre_shots <- shots %>%
  filter(name == 'Andre Iguodala' & x >= -100 & x <=100 & y <= 100) %>%
  group_by(shot_made_flag) %>%
  summarise(Andre = n())
andre_shot_types <- shots %>%
  filter(name == 'Andre Iguodala') %>%
  group_by(action_type) %>%
  summarise(Andre = n())

dray_shots <- shots %>%
  filter(name == 'Draymond Green' & x >= -100 & x <=100 & y <= 100) %>%
  group_by(shot_made_flag) %>%
  summarise(Draymond = n())
dray_shot_types <- shots %>%
  filter(name == 'Draymond Green') %>%
  group_by(action_type) %>%
  summarise(Draymond = n())

curry_shots <- shots %>%
  filter(name == 'Stephen Curry' & x >= -100 & x <=100 & y <= 100) %>%
  group_by(shot_made_flag) %>%
  summarise(Stephen = n())
curry_shot_types <- shots %>%
  filter(name == 'Stephen Curry') %>%
  group_by(action_type) %>%
  summarise(Stephen = n())

kevin_shots <- shots %>%
  filter(name == 'Kevin Durant' & x >= -100 & x <=100 & y <= 100) %>%
  group_by(shot_made_flag) %>%
  summarise(Durant = n())
kevin_shot_types <- shots %>%
  filter(name == 'Kevin Durant') %>%
  group_by(action_type) %>%
  summarise(Durant = n())

klay_shots <- shots %>%
  filter(name == 'Klay Thompson' & x >= -100 & x <=100 & y <= 100) %>%
  group_by(shot_made_flag) %>%
  summarise(Thompson = n())
klay_shot_types <- shots %>%
...
```{r echo = FALSE}
ggplot(PT, aes(x = total, y = made)) + geom_point() + geom_smooth(method = 'lm') 
```
...
```{r echo = FALSE}
ggplot(PT2, aes(x = total, y = made)) + geom_point() + geom_smooth(method = 'lm') 
```
...
```{r echo = FALSE, eval=FALSE}
tble2 <- full_join(dray_shot_types, andre_shot_types, by = 'action_type')
tble2 <- full_join(tble2, curry_shot_types, by = 'action_type')
tble2 <- full_join(tble2, klay_shot_types, by = 'action_type')
tble2 <- full_join(tble2, kevin_shot_types, by = 'action_type')
tble2 <- rename(tble2, ActionType = action_type)
kable(tble2)
```
